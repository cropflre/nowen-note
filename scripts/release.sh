...
# ===== Linux 桌面端架构选择（仅 PC_HAS_LINUX=1 时生效，实验性）=====
# 逗号分隔，可选 x64 / arm64；默认 x64。
# Linux arm64 AppImage/deb 仍需在真实 ARM64 桌面环境验证后再进入正式 Release 矩阵。
LINUX_ARCHES=""        # --linux-arch x64,arm64
...
多端发版选项（可组合）:
      ...
      --upk-build N        绿联 .upk 的构建号（默认 1，会拼成 X.Y.Z.N 写入包名）
      ...
      --linux-arch LIST    Linux 桌面端 CPU 架构，逗号分隔：x64 / arm64（默认 x64）
                           arm64 属于 experimental，本地验证通过前不会进入正式 Release 矩阵
...
if [ "$HAS_PC" = "1" ]; then
    ...
    # ---- Linux 桌面端架构选择 ----
    # 默认只打 x64，arm64 作为显式 opt-in 的 experimental 构建入口。
    # 每个架构会独立 rebuild native，再独立调用 electron-builder，避免 .node 错架构。
    LINUX_ARCHES="$(echo "${LINUX_ARCHES:-x64}" | tr ',' '\n' | awk 'NF && !seen[$0]++{print}' | tr '\n' ',' | sed 's/,$//')"
    if [ "$PC_HAS_LINUX" = "1" ]; then
        for la in $(echo "$LINUX_ARCHES" | tr ',' ' '); do
            case "$la" in
                x64|arm64) ;;
                *) die "--linux-arch 未知值: $la （合法: x64 / arm64 / x64,arm64）" ;;
            esac
        done
        info "Linux PC 架构: ${LINUX_ARCHES}"
    fi
...
if [ "$HAS_PC" = "1" ]; then
    ...
    if [ "$UNAME_S_PC" = "Linux" ] || [ "$UNAME_S_PC" = "Darwin" ]; then
        ...
        _build_one_pc_target() {
            ...
            local _eb_args=( "${_eb_flag}" )
            if [ "$_eb_flag" = "--linux" ]; then
                _eb_args+=( "--${_target_arch}" )
            fi
            ...
            if [ "$DRY_RUN" = "1" ]; then
                ...
                echo "  (dry-run) npx electron-builder --config electron/builder.config.js --publish never ${_eb_args[*]}"
                return 0
            fi
        ...
        if [ "$PC_HAS_LINUX" = "1" ]; then
            for _la in $(echo "$LINUX_ARCHES" | tr ',' ' '); do
                info "linux arch=${_la}: 先 rebuild:native，再 electron-builder（experimental）"
                _build_one_pc_target --linux linux "$_la"
            done
            if [ "$(echo "$LINUX_ARCHES" | tr ',' '\n' | wc -l | tr -d ' ')" -gt 1 ]; then
                warn "本次打了多个 Linux 架构（${LINUX_ARCHES}），latest-linux.yml 仅保留最后一次构建的架构信息——自动更新仅对该架构生效"
            fi
        fi
