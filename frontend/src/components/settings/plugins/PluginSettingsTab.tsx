...
type PendingPackage = { file: File; manifest: PendingManifest };
...
export default function PluginSettingsTab({ isAdmin }: { isAdmin: boolean }) {
  ...
  useEffect(() => {
    ...
    void pluginApi.registrySources().then(setSources).catch(() => setSources([]));
  ...
  const choosePackage = async (file?: File) => {
    ...
    try {
      ...
      setPending({ file, manifest: JSON.parse(await manifestFile.async("string")) as PendingManifest });
  ...
  return <div className="space-y-5">
    ...
    {tab === "developer" && (isAdmin && developerModeAvailable ? <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 p-4 space-y-3"><div className="flex items-center justify-between"><div><h3 className="flex items-center gap-2 text-sm font-semibold"><Code2 size={15} />开发者模式</h3><p className="mt-1 text-xs text-zinc-500">本地目录仍需 Validate、Preflight 和权限确认；开发插件不进入备份。</p></div><input type="checkbox" checked={developerMode} onChange={async (event) => { const enabled = event.target.checked; await pluginApi.setDeveloperMode(enabled); setDeveloperMode(enabled); }} /></div>{developerMode && <div className="flex gap-2"><input value={devDirectory} onChange={(event) => setDevDirectory(event.target.value)} placeholder="D:\\Projects\\nowen-plugin-test" className="min-w-0 flex-1 rounded-md border border-zinc-200 dark:border-zinc-700 bg-transparent px-3 py-2 text-xs" /><button onClick={async () => { await pluginApi.loadDevelopment(devDirectory); await refresh(); setTab("installed"); }} className="rounded-md border px-3 py-2 text-xs">Validate & Load</button></div>}</div> : <div className="rounded-xl border border-dashed py-10 text-center text-sm text-zinc-500 dark:border-zinc-700">开发者模式仅对管理员和本地 Desktop Backend 开放。</div>)}
    {tab === "developer" && isAdmin && <div className="rounded-xl border p-4 dark:border-zinc-800"><h3 className="text-sm font-semibold">Registry 来源与国内镜像</h3><p className="mt-1 text-xs text-zinc-500">协议是开放 JSON，可添加 HTTPS 镜像或自建 Registry；安装时仍校验 SHA256。</p><div className="mt-3 space-y-1">{sources.map((source) => <div key={source.id} className="truncate rounded bg-zinc-100 px-2 py-1.5 text-[11px] dark:bg-zinc-900">{source.name} · {source.url}</div>)}</div><div className="mt-3 grid gap-2 sm:grid-cols-[1fr_2fr_auto]"><input value={newSourceName} onChange={(event) => setNewSourceName(event.target.value)} placeholder="镜像名称" className="rounded-md border bg-transparent px-2 py-1.5 text-xs dark:border-zinc-700" /><input value={newSourceUrl} onChange={(event) => setNewSourceUrl(event.target.value)} placeholder="https://mirror.example/registry.json" className="rounded-md border bg-transparent px-2 py-1.5 text-xs dark:border-zinc-700" /><button onClick={async () => { const custom = sources.filter((source) => !source.official); const id = `mirror-${Date.now().toString(36)}`; const updated = await pluginApi.setRegistrySources([...custom, { id, name: newSourceName || "Custom Registry", url: newSourceUrl }]); setSources(updated); setNewSourceName(""); setNewSourceUrl(""); }} className="rounded-md border px-3 py-1.5 text-xs">添加</button></div></div>}
    {pending && <div className="fixed inset-0 z-[10001] flex items-center justify-center bg-black/60 p-4"><div className="w-full max-w-lg rounded-2xl bg-white dark:bg-zinc-950 p-5 shadow-2xl space-y-4"><div><h3 className="text-lg font-bold">安装 {pending.manifest.name || "社区插件"}</h3><p className="mt-1 font-mono text-xs text-zinc-500">{pending.manifest.id} · v{pending.manifest.version}</p></div><div className="rounded-lg border border-amber-300/60 bg-amber-50 dark:bg-amber-950/20 p-3 text-sm text-amber-800 dark:text-amber-300"><div className="flex gap-2"><AlertTriangle className="mt-0.5 shrink-0" size={16} /><span>此插件包含第三方可执行代码。Node 子进程提供故障隔离，但不是真正安全沙箱；请仅安装你信任的来源。</span></div></div><div><h4 className="mb-2 text-sm font-semibold">它希望获得：</h4><ul className="space-y-1 text-sm">{pending.manifest.permissions?.length ? pending.manifest.permissions.map((permission: string) => <li key={permission}>✓ {permissionLabels[permission] || permission}</li>) : <li className="text-zinc-500">不请求数据权限</li>}</ul>{(pending.manifest.permissionConfig?.externalFetchHosts?.length ?? 0) > 0 && <p className="mt-2 text-xs text-zinc-500">网络访问：{pending.manifest.permissionConfig?.externalFetchHosts?.join(", ")}</p>}</div><p className="text-xs text-zinc-500">安装后默认进入隔离状态；你需要再次确认权限并启用。</p><div className="flex justify-end gap-2"><button onClick={() => setPending(null)} className="rounded-md border px-3 py-2 text-sm">取消</button><button disabled={installing} onClick={install} className="rounded-md bg-indigo-600 px-3 py-2 text-sm text-white disabled:opacity-50">{installing ? "正在验证" : "信任并安装"}</button></div></div></div>}
  </div>;
}
