...
function PluginCard({ plugin, isAdmin, refresh }: { plugin: InstalledPlugin; isAdmin: boolean; refresh: () => Promise<void> }) {
  ...
  const runtimePresentation = getPluginRuntimePresentation(plugin);
  ...
  const toggleDetails = async () => {
    ...
    if (next) {
      const [runs, configured, settings] = await Promise.all([
        runtimePresentation.declarative ? Promise.resolve([] as PluginExecution[]) : pluginApi.executions(plugin.id).catch(() => []),
        runtimePresentation.declarative ? Promise.resolve([] as PluginConnection[]) : pluginApi.connections(plugin.id).catch(() => []),
        pluginApi.settings(plugin.id).catch(() => ({})),
      ]);
  ...
  return <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-4 space-y-3">
    ...
    <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-500">
      ...
      <span className="inline-flex items-center gap-1 rounded-full bg-zinc-100 px-2 py-0.5 dark:bg-zinc-900"><ShieldCheck size={11} />{plugin.trustLevel}</span>
      {plugin.signatureState === "verified" && <span className="rounded-full bg-emerald-500/15 px-2 py-0.5 text-emerald-600">签名已验证</span>}
      {plugin.advisoryState && plugin.advisoryState !== "unknown" && <span className="rounded-full bg-red-500/15 px-2 py-0.5 text-red-600">安全：{plugin.advisoryState}</span>}
      {runtimePresentation.declarative ? <>
        <span className="rounded-full bg-indigo-500/10 px-2 py-0.5 text-indigo-600 dark:text-indigo-300">{runtimePresentation.runtimeLabel}</span>
        <span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-emerald-600 dark:text-emerald-300">{runtimePresentation.executionLabel}</span>
        <span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-emerald-600 dark:text-emerald-300">{runtimePresentation.permissionLabel}</span>
      </> : <span>{runtimePresentation.executionLabel}</span>}
    </div>
    {runtimePresentation.declarative && <div className="rounded-md border border-emerald-200/70 bg-emerald-50 p-2.5 text-xs leading-relaxed text-emerald-800 dark:border-emerald-900/60 dark:bg-emerald-950/20 dark:text-emerald-300"><div className="flex gap-2"><ShieldCheck className="mt-0.5 shrink-0" size={14} /><span>此扩展只注册声明式资源：不会启动 Worker / Node / QuickJS，不可执行 Action，也不能申请笔记、附件、网络或密钥数据权限。</span></div></div>}
    ...
    <div className="flex flex-wrap gap-2">
      ...
      {isAdmin && plugin.status !== "enabled" && <button disabled={busy} onClick={() => act(async () => { if (runtimePresentation.requiresPermissionConfirmation) await pluginApi.grant(plugin.id, plugin.permissions.map((item) => item.permission)); await pluginApi.enable(plugin.id); })} className="rounded-md bg-indigo-600 px-2.5 py-1.5 text-xs text-white">{runtimePresentation.enableLabel}</button>}
      {isAdmin && !runtimePresentation.declarative && <button disabled={busy} onClick={() => act(() => pluginApi.reload(plugin.id))} className="rounded-md border px-2.5 py-1.5 text-xs disabled:opacity-40"><RefreshCw size={12} /></button>}
    ...
    {expanded && <div className="space-y-4 border-t border-zinc-200 dark:border-zinc-800 pt-3">
      <div><h4 className="mb-2 text-xs font-semibold">权限</h4><ul className="space-y-1">{plugin.permissions.length ? plugin.permissions.map((item) => <li key={item.permission} className="flex items-center gap-2 text-xs text-zinc-600 dark:text-zinc-400">{item.granted ? <CheckCircle2 size={13} className="text-emerald-500" /> : <AlertTriangle size={13} className="text-amber-500" />}{permissionLabels[item.permission] || item.permission}<span className="ml-auto font-mono text-[10px] text-zinc-400">{item.permission}</span></li>) : <li className="text-xs text-zinc-500">{runtimePresentation.declarative ? "零数据权限（Manifest 安装阶段强制）" : "不请求数据权限"}</li>}</ul></div>
      {runtimePresentation.canExecuteActions ? <div className="space-y-2"><h4 className="text-xs font-semibold">Actions</h4>{plugin.actions.map((action) => <ActionTester key={action.id} plugin={plugin} action={action} />)}</div> : <div className="rounded-lg bg-zinc-50 p-3 text-xs text-zinc-500 dark:bg-zinc-900/60">执行能力：无。声明式扩展不能执行 Action，也不会创建代码 Runner。</div>}
      ...
      <div><h4 className="mb-2 text-xs font-semibold">最近执行</h4>{runtimePresentation.declarative ? <p className="text-xs text-zinc-500">声明式扩展没有执行进程或执行记录。</p> : executions.length ? <div className="max-h-48 overflow-auto space-y-1">{executions.map((item) => { const progress = item.progressTotal ? Math.min(100, Math.round(((item.progressCurrent || 0) / item.progressTotal) * 100)) : null; return <div key={item.id} className="rounded bg-zinc-100 dark:bg-zinc-900 p-2 text-[11px]"><span className="font-mono">{item.actionId}</span> · {item.status} · {item.durationMs ?? "—"}ms{progress !== null && <div className="mt-1"><div className="h-1.5 overflow-hidden rounded bg-zinc-200 dark:bg-zinc-800"><div className="h-full bg-indigo-500" style={{ width: `${progress}%` }} /></div><div className="mt-1 text-zinc-500">{item.progressMessage || `${item.progressCurrent || 0} / ${item.progressTotal}`}</div></div>}{item.errorMessage && <div className="mt-1 text-red-500">{item.errorMessage}</div>}</div>; })}</div> : <p className="text-xs text-zinc-500">暂无执行记录</p>}</div>
    </div>}
  </div>;
}
...
export default function PluginSettingsTab({ isAdmin }: { isAdmin: boolean }) {
  ...
  const pendingRuntimePresentation = pending ? getPluginRuntimePresentation(pending.manifest) : null;
  ...
  return <div className="space-y-5">
    <div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-bold">扩展生态</h2><p className="mt-1 text-sm text-zinc-500">V2 可执行插件默认运行在 QuickJS/WASM 沙箱；声明式扩展不执行代码、零数据权限。签名、权限变化和安全公告会在安装前核验。</p></div>{isAdmin && tab === "installed" && <><input ref={inputRef} type="file" accept=".nowen-plugin" className="hidden" onChange={(event) => void choosePackage(event.target.files?.[0])} /><button onClick={() => inputRef.current?.click()} className="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-2 text-xs font-medium text-white"><PackagePlus size={14} />安装插件</button></>}</div>
    ...
    {tab === "discover" && <div className="space-y-3"><div className="flex gap-2"><div className="relative flex-1"><Search size={14} className="absolute left-3 top-2.5 text-zinc-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索插件、分类或关键词" className="w-full rounded-lg border bg-transparent py-2 pl-9 pr-3 text-sm dark:border-zinc-700" /></div><select value={sourceId} onChange={(event) => setSourceId(event.target.value)} className="rounded-lg border bg-transparent px-2 text-xs dark:border-zinc-700">{sources.map((source) => <option key={source.id} value={source.id}>{source.name}</option>)}</select></div>{catalogLoading ? <div className="flex items-center gap-2 py-10 text-sm text-zinc-500"><Loader2 size={15} className="animate-spin" />加载 Registry</div> : catalogError ? <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-8 text-center text-sm text-red-700 dark:border-red-900/60 dark:bg-red-950/20 dark:text-red-300"><div className="font-medium">Registry 加载失败</div><div className="mt-1 text-xs opacity-80">{catalogError}</div></div> : visibleCatalog.length ? <div className="grid gap-3 md:grid-cols-2">{visibleCatalog.map((plugin) => <div key={plugin.id} className="rounded-xl border p-4 dark:border-zinc-800"><div className="flex items-start justify-between gap-2"><div><h3 className="font-semibold">{plugin.name}</h3><p className="font-mono text-[10px] text-zinc-400">{plugin.id}</p></div><span className="rounded-full bg-zinc-100 px-2 py-1 text-[10px] dark:bg-zinc-900">{plugin.trustLevel || "community"}</span></div><p className="mt-2 text-xs text-zinc-500">{plugin.description || "暂无说明"}</p><div className="mt-3 flex items-center justify-between text-xs"><span>v{plugin.latestVersion}</span>{isAdmin && <button onClick={async () => { await pluginApi.installFromRegistry(sourceId, plugin.id); setTab("installed"); await refresh(); }} className="inline-flex items-center gap-1 rounded-md bg-indigo-600 px-2.5 py-1.5 text-white"><Download size={12} />安装</button>}</div></div>)}</div> : <div className="rounded-xl border border-dashed py-10 text-center text-sm text-zinc-500 dark:border-zinc-700">Registry 暂无可显示插件。</div>}</div>}
    ...
    {pending && pendingRuntimePresentation && <div className="fixed inset-0 z-[10001] flex items-center justify-center bg-black/60 p-4"><div className="w-full max-w-lg rounded-2xl bg-white dark:bg-zinc-950 p-5 shadow-2xl space-y-4"><div><h3 className="text-lg font-bold">安装 {pending.manifest.name || "社区插件"}</h3><p className="mt-1 font-mono text-xs text-zinc-500">{pending.manifest.id} · v{pending.manifest.version}</p></div>{pendingRuntimePresentation.declarative ? <div className="rounded-lg border border-emerald-200/70 bg-emerald-50 p-3 text-sm text-emerald-800 dark:border-emerald-900/60 dark:bg-emerald-950/20 dark:text-emerald-300"><div className="flex gap-2"><ShieldCheck className="mt-0.5 shrink-0" size={16} /><span>这是声明式零代码扩展：不会运行 JS / WASM / Node，不执行 Action，也不能请求笔记、附件、网络或密钥数据权限。安装仍会校验签名、校验和与兼容性。</span></div></div> : <div className="rounded-lg border border-amber-300/60 bg-amber-50 p-3 text-sm text-amber-800 dark:bg-amber-950/20 dark:text-amber-300"><div className="flex gap-2"><AlertTriangle className="mt-0.5 shrink-0" size={16} /><span>{pending.nodeRuntimeConfirmationRequired ? "此 V2 插件将在独立 Node 进程中执行二进制/Node 代码，可使用获准的文件系统与网络权限，且不能由 QuickJS/WASM Sandbox 隔离。请仅在完全信任来源时继续。" : "此插件包含第三方可执行代码；安装前将校验兼容性、信任等级和 Runtime 策略。"}</span></div></div>}<div><h4 className="mb-2 text-sm font-semibold">权限与执行边界：</h4><ul className="space-y-1 text-sm">{pending.manifest.permissions?.length ? pending.manifest.permissions.map((permission: string) => <li key={permission}>✓ {permissionLabels[permission] || permission}</li>) : <li className="text-zinc-500">{pendingRuntimePresentation.declarative ? "零数据权限（由 Manifest 校验强制保证）" : "不请求数据权限"}</li>}</ul>{(pending.manifest.permissionConfig?.externalFetchHosts?.length ?? 0) > 0 && <p className="mt-2 text-xs text-zinc-500">网络访问：{pending.manifest.permissionConfig?.externalFetchHosts?.join(", ")}</p>}</div><p className="text-xs text-zinc-500">{pendingRuntimePresentation.declarative ? "安装后默认进入隔离状态；启用只会注册声明资源，不会启动代码 Runner。" : pending.nodeRuntimeConfirmationRequired ? "确认后将使用同一文件重新提交；取消则不会安装。" : "安装后默认进入隔离状态；你需要再次确认权限并启用。"}</p><div className="flex justify-end gap-2"><button onClick={() => { setPending(null); if (inputRef.current) inputRef.current.value = ""; }} className="rounded-md border px-3 py-2 text-sm">取消</button><button disabled={installing} onClick={install} className="rounded-md bg-indigo-600 px-3 py-2 text-sm text-white disabled:opacity-50">{installing ? "正在验证" : pendingRuntimePresentation.declarative ? "安装声明式扩展" : pending.nodeRuntimeConfirmationRequired ? "确认 Node 风险并安装" : "继续安装"}</button></div></div></div>}
  </div>;
}
