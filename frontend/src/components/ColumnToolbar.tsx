/**
 * ColumnToolbar —— 分栏（columns）的语雀式操作浮层。
 *
 * 交互：鼠标悬停到分栏的某一栏时，在该栏正上方浮出两个 16px 微圆点按钮：
 *   - 水平居中的 ⋯（更多）：弹出菜单 → 单栏时「删除分栏」，多栏时「删除该栏」
 *   - 右缘的 ＋：在命中栏后追加一栏（≤6 栏）
 * 点击外部即关闭菜单。
 *
...
export function ColumnToolbar({ editor, editable }: ColumnToolbarProps) {
  ...
  return (
    ...
    >
      ...
      {menu && (
        <div
          className="absolute min-w-[8rem] bg-app-elevated border border-app-border rounded-lg shadow-lg p-1 z-20"
