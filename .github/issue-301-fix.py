...
drop_start_marker = '''      handleDrop: (view, event, _slice, moved) => {'''
drop_end_marker = '''        return true;
      },'''
drop_start = source.index(drop_start_marker)
drop_end = source.index(drop_end_marker, drop_start) + len(drop_end_marker)
new_drop = '''      handleDrop: (view, event, _slice, moved) => {
...
source = source[:drop_start] + new_drop + source[drop_end:]

handlers_start_marker = '''  const handleImageUpload = useCallback(() => {'''
handlers_end_marker = '''  /**
   * 严格作用于当前选区的代码块切换：'''
handlers_start = source.index(handlers_start_marker)
handlers_end = source.index(handlers_end_marker, handlers_start)
new_handlers = '''  const captureEditorInsertAnchor = useCallback(() => {
...
Path("frontend/src/lib/asyncEditorInsert.ts").write_text(
...
test_dir = Path("frontend/src/lib/__tests__")
test_dir.mkdir(parents=True, exist_ok=True)
(test_dir / "asyncEditorInsert.test.ts").write_text(
