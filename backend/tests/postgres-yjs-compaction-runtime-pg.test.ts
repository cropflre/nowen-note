...
async function insertNoteFixture(
  ...
  for (let index = 0; index < fixture.updates.length; index += 1) {
    const blob = corruptLastUpdate && index === fixture.updates.length - 1
      ? Buffer.from([1, 2, 3])
...
test("PostgreSQL Yjs compaction advances a safe watermark, retains a row margin and remains recoverable", { skip: !hasPg }, async () => {
  ...
  await insertNoteFixture(pool, CORRUPT_NOTE, corruptFixture, true);
  ...
  try {
    ...
    assert.equal(first.scannedNotes, 3);
    assert.equal(first.compactedNotes, 2);
    assert.equal(first.blockedNotes, 1);
    ...
    assert.equal(Number(corruptState.updatesMergedTo), 0);
    ...
    const failingAdapter: DatabaseAdapter = {
      ...
      executeStatements: (statements) => adapter.executeStatements(statements),
    };
    await pool.query(
    ...
    assert.equal(stats.blockedNotes, 1);
  } finally {
    await runtime.close();
