...
/**
 * Resolve all notebooks hidden from this user's global search in one recursive query. The result
 * is intentionally a Set that can be reused by every candidate source during one search request,
 * avoiding per-result ancestor lookups and keeping excluded archive hits from consuming candidate
 * limits before active notes are considered.
 */
