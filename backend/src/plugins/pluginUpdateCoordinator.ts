...
export class PluginUpdateCoordinator {
  ...
  async installUpdate(validated: ValidatedPluginPackage, requestedBy: string, provenance: PluginUpdateProvenance = {}): Promise<PluginRegistryRecord> {
    ...
    try {
      ...
      if (coded.message.includes("UNIQUE constraint failed")) throw Object.assign(new Error("该插件已有未完成的更新操作"), { code: "PLUGIN_UPDATE_IN_PROGRESS" });
      throw error;
    }
    ...
    try {
      ...
      getDb().prepare("UPDATE plugin_update_operations SET stagingPath=?,updatedAt=? WHERE id=? AND stage='verified'").run(stagingPath, timestamp(), operationId);
      ...
      getDb().transaction(() => {
        this.registry.registerVersion({
          manifest: validated.manifest, checksum: validated.checksum, installedPath: committedPath,
          source: provenance.source || "package", trustLevel: provenance.trustLevel || "community", status: "installed",
          publisherKeyId: provenance.publisherKeyId, signature: provenance.signature, signatureState: provenance.signatureState, artifactUrl: provenance.artifactUrl,
        });
      ...
      const candidate = this.registry.recordForVersion(current.id, validated.manifest.version, { nodeRuntimeConfirmedBy: provenance.nodeRuntimeConfirmedBy });
      ...
      const declarative = isDeclarativePluginManifest(validated.manifest);
      const activated = this.lifecycle.activateCandidate(current.id, operationId, declarative ? 1 : 5, {
        confirmedAt: candidate.nodeRuntimeConfirmedAt,
        confirmedBy: candidate.nodeRuntimeConfirmedBy,
      });
      if (declarative) {
        this.lifecycle.completeProbationExecution(current.id);
      ...
      } else this.lifecycle.markOperationFailed(operationId, coded.code || "PLUGIN_UPDATE_FAILED", coded.message);
  ...
  async activateExistingVersion(pluginId: string, targetVersion: string, requestedBy: string): Promise<PluginRegistryRecord> {
    ...
    if (!current || !target || current.version === target.version) throw Object.assign(new Error("没有可回滚版本"), { code: "PLUGIN_VERSION_NOT_FOUND" });
    if (target.status !== "stable" || !target.verifiedAt) throw Object.assign(new Error("只能切换到已验证的 stable 历史版本"), { code: "PLUGIN_VERSION_NOT_VERIFIED_STABLE" });
    if (!fs.existsSync(target.installedPath)) throw Object.assign(new Error("回滚版本文件不存在"), { code: "PLUGIN_VERSION_FILES_MISSING" });
    const operationId = crypto.randomUUID();
    const createdAt = timestamp();
    ...
    try {
      ...
      await this.executions.preflightCandidate(candidate);
      const targetManifest = JSON.parse(target.manifestJson) as PluginManifest;
      this.permissions.initialize(targetManifest, { preserveExistingGrants: true });
      const declarative = isDeclarativePluginManifest(targetManifest);
      const activated = this.lifecycle.activateCandidate(pluginId, operationId, declarative ? 1 : 5, {
        confirmedAt: candidate.nodeRuntimeConfirmedAt,
        confirmedBy: candidate.nodeRuntimeConfirmedBy,
      });
      if (declarative) {
        this.lifecycle.completeProbationExecution(pluginId);
      ...
      } else this.lifecycle.markOperationFailed(operationId, coded.code || "PLUGIN_ROLLBACK_FAILED", coded.message);
      throw error;
    } finally {
