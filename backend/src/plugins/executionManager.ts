...
export class PluginExecutionManager {
  ...
  private runner(
    ...
    if (record.runtime === "declarative") throw Object.assign(new Error("声明式插件不执行代码"), { code: "PLUGIN_DECLARATIVE_NOT_EXECUTABLE" });
    if (!allowPreflight && this.suspendedForUpdate.has(pluginId)) throw Object.assign(new Error("插件正在切换版本"), { code: "PLUGIN_UPDATE_IN_PROGRESS" });
    if (!allowPreflight && record.activeOperationId && record.lifecycleState !== "probation") throw Object.assign(new Error("插件正在验证候选版本"), { code: "PLUGIN_UPDATE_IN_PROGRESS" });
    if (!allowPreflight && record.status !== "enabled") throw Object.assign(new Error(`插件当前状态为 ${record.status}`), { code: "PLUGIN_NOT_ENABLED" });
    if (record.lifecycleState !== "stable" && record.lifecycleState !== "probation" && !(allowPreflight && record.lifecycleState === "preflight")) {
  ...
  private async acquireProbationSlot(pluginId: string): Promise<() => void> {
    ...
    } else this.probationActive.add(pluginId);
    let released = false;
    return () => {
      if (released) return;
      released = true;
  ...
  private assertQueuedBinding(pluginId: string, actionId: string, queued: { version: string; checksum: string; actionDigest: string }): PluginRegistryRecord {
    const current = this.registry.get(pluginId);
    if (!current) throw Object.assign(new Error("插件不存在"), { code: "PLUGIN_NOT_FOUND" });
    if (current.version !== queued.version || current.checksum !== queued.checksum) throw Object.assign(new Error("插件在排队期间已切换版本"), { code: "PLUGIN_VERSION_CHANGED_WHILE_QUEUED" });
    if (actionSchemaDigest(current, actionId) !== queued.actionDigest) throw Object.assign(new Error("Action schema 在排队期间已变化"), { code: "PLUGIN_ACTION_SCHEMA_CHANGED" });
    return current;
  }
  ...
  async execute(input: {
    ...
    if (queuedRecord.runtime === "declarative") throw Object.assign(new Error("声明式插件不执行 Action"), { code: "PLUGIN_DECLARATIVE_NOT_EXECUTABLE" });
    const queuedBinding = { version: queuedRecord.version, checksum: queuedRecord.checksum, actionDigest: actionSchemaDigest(queuedRecord, input.actionId) };
    const executionId = input.executionId || crypto.randomUUID();
    let startedAt = new Date();
    const context: PluginExecutionContext = {
      executionId, pluginId: input.pluginId, actionId: input.actionId, userId: input.userId,
      workspaceId: input.workspaceId || null, ...(input.executionContext || {}),
    };
    ...
    try {
      ...
      if (outputBytes > PACKAGE_LIMITS.outputBytes) throw Object.assign(new Error("Action output 超过 1MB"), { code: "PLUGIN_OUTPUT_TOO_LARGE" });
      ...
      if (shouldRollbackProbation) {
        ...
        } finally { this.resumeAfterUpdate(input.pluginId); }
      } else if (coded.code === "PLUGIN_TIMEOUT") this.registry.setStatus(input.pluginId, "error", coded.message);
  ...
  list(pluginId: string, userId?: string, limit = 100): Record<string, unknown>[] {
    if (userId) return getDb().prepare("SELECT * FROM plugin_executions WHERE pluginId=? AND userId=? ORDER BY startedAt DESC LIMIT ?").all(pluginId, userId, Math.min(500, Math.max(1, limit))) as Record<string, unknown>[];
    return getDb().prepare("SELECT * FROM plugin_executions WHERE pluginId=? ORDER BY startedAt DESC LIMIT ?").all(pluginId, Math.min(500, Math.max(1, limit))) as Record<string, unknown>[];
  }
  ...
  async preflight(pluginId: string): Promise<void> {
    const record = this.registry.get(pluginId);
    if (record?.runtime === "declarative") return;
  ...
  async preflightCandidate(record: PluginRegistryRecord): Promise<void> {
    if (record.runtime === "declarative") return;
    const resolvedRuntime = this.policy.assertAllowed(compatibilityInputFromRecord(record));
    if (resolvedRuntime === "declarative") return;
    const Runner = resolvedRuntime === "sandbox-js" ? SandboxRunner : PluginRunner;
    const runner = new Runner(record, (context, call) => this.broker.call(context, call), () => undefined);
    try { await runner.preflight(); }
    finally { await runner.terminate(); }
  }

  suspendForUpdate(pluginId: string): void { this.suspendedForUpdate.add(pluginId); }
  resumeAfterUpdate(pluginId: string): void { this.suspendedForUpdate.delete(pluginId); }
  ...
  private updateProgress(executionId: string, progress: { current?: number; total?: number; message?: string }): void {
    ...
    getDb().prepare(`UPDATE plugin_executions SET progressCurrent=?,progressTotal=?,progressMessage=? WHERE id=? AND status='running'`)
      .run(current, total, progress.message?.slice(0, 500) || null, executionId);
  }

  async shutdown(pluginId: string): Promise<void> { await this.restart(pluginId); }
}
