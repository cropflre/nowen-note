...
export function syncAutomaticNoteLinkTitles(
  ...
  for (const row of rows) {
    ...
    if (!doc || typeof doc !== "object" || !Array.isArray(doc.content)) continue;
      ...
      for (const node of nodes) {
        ...
        if (node.type === "text" && Array.isArray(node.marks)) {
          for (const mark of node.marks) {
            ...
            if (targetIdFromHref(mark?.attrs?.href) !== targetNoteId.toLowerCase()) continue;
    ...
    const contentText = plainTextFromNoteContent(normalized, "tiptap-json");
    db.prepare(`
      UPDATE notes
      SET content = ?, contentText = ?, version = version + 1, updatedAt = datetime('now')
      WHERE id = ?
    `).run(normalized, contentText, row.id);
    const synced = syncNoteBlocks(db, row.id, normalized, "tiptap-json");
